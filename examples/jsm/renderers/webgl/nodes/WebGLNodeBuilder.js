import NodeBuilder, {
	defaultShaderStages,
} from "../../../nodes/core/NodeBuilder.js";
...
class WebGLNodeBuilder extends NodeBuilder {
	...
	_parseObject() {
		...
		if (renderer.toneMappingNode?.isNode === true) {
			this.replaceCode(
				"fragment",
				getIncludeSnippet("tonemapping_fragment"),
				""
			);
		}
		...
		if (material.colorNode && material.colorNode.isNode) {
			this.addSlot(
				"fragment",
				new SlotNode(material.colorNode, "COLOR", "vec4")
			);
		}
		...
		if (material.opacityNode && material.opacityNode.isNode) {
			this.addSlot(
				"fragment",
				new SlotNode(material.opacityNode, "OPACITY", "float")
			);
		}
		...
		if (material.normalNode && material.normalNode.isNode) {
			this.addSlot(
				"fragment",
				new SlotNode(material.normalNode, "NORMAL", "vec3")
			);
		}
		...
		if (material.emissiveNode && material.emissiveNode.isNode) {
			this.addSlot(
				"fragment",
				new SlotNode(material.emissiveNode, "EMISSIVE", "vec3")
			);
		}
		...
		if (material.metalnessNode && material.metalnessNode.isNode) {
			this.addSlot(
				"fragment",
				new SlotNode(material.metalnessNode, "METALNESS", "float")
			);
		}
		...
		if (material.roughnessNode && material.roughnessNode.isNode) {
			this.addSlot(
				"fragment",
				new SlotNode(material.roughnessNode, "ROUGHNESS", "float")
			);
		}
		...
		if (material.clearcoatNode && material.clearcoatNode.isNode) {
			this.addSlot(
				"fragment",
				new SlotNode(material.clearcoatNode, "CLEARCOAT", "float")
			);
		}
		...
		if (material.iridescenceNode && material.iridescenceNode.isNode) {
			this.addSlot(
				"fragment",
				new SlotNode(material.iridescenceNode, "IRIDESCENCE", "float")
			);
		}
		...
		if (material.iridescenceIORNode && material.iridescenceIORNode.isNode) {
			this.addSlot(
				"fragment",
				new SlotNode(material.iridescenceIORNode, "IRIDESCENCE_IOR", "float")
			);
		}
		...
		if (material.envNode && material.envNode.isNode) {
			const envRadianceNode = new WebGLPhysicalContextNode(
				WebGLPhysicalContextNode.RADIANCE,
				material.envNode
			);
			const envIrradianceNode = new WebGLPhysicalContextNode(
				WebGLPhysicalContextNode.IRRADIANCE,
				material.envNode
			);

			this.addSlot(
				"fragment",
				new SlotNode(envRadianceNode, "RADIANCE", "vec3")
			);
			this.addSlot(
				"fragment",
				new SlotNode(envIrradianceNode, "IRRADIANCE", "vec3")
			);
		}
		...
		if (material.positionNode && material.positionNode.isNode) {
			this.addSlot(
				"vertex",
				new SlotNode(material.positionNode, "POSITION", "vec3")
			);
		}
